...
def ifc_schema_graph_main():
    ...
    for epoch in range(1, isg_Config['num_epochs'] + 1):
        trainer.train(epoch, isg_Config["in_feat"], isg_Config['temperature'], isg_Config['max_norm'], isg_Config["save_path"])
        scheduler.step()
...
def ifc_file_graph_main():
    ...
    for epoch in range(1, ifg_Config['num_epochs'] + 1):
        ...
        if epoch % ifg_Config['valid_freq'] == 0:
            tester.test(epoch, device, ifg_Config['test_batch_size'], ifg_Config['save_path'])
...
def mvcnn_main():
    ...
    for epoch in range(1, img_Config['num_epochs'] + 1):
        ...
        if epoch % img_Config['valid_freq'] == 0:
            tester.test(epoch, device, img_Config['test_batch_size'], img_Config['save_path'])
